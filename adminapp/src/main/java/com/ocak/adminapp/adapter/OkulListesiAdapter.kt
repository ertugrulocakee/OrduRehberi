...
class OkulListesiAdapter(var okulList : ArrayList<Okul>) : RecyclerView.Adapter<OkulListesiAdapter.OkulListesiViewHolder>(){
    ...
    override fun onBindViewHolder(holder: OkulListesiViewHolder, position: Int) {
        ...
        holder.itemView.silOkulButonu.setOnClickListener {

            val string = okulList.get(position).okulId

            database.collection("Okullar").document(string).delete().addOnCompleteListener {
                if (it.isSuccessful) {
                    Toast.makeText(holder.itemView.context, "Seçilen okul başarıyla silindi", Toast.LENGTH_LONG).show()
                }
